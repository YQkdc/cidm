...
def loss_fn(model, Input_Data, T, cond, cat_embedding, device, eps=1e-5):
    N, input_dim = Input_Data.shape  
    loss_values = torch.empty(N, device=device)
    for row in range(N):
        ...
        score = model(perturbed_data, random_t, cond[row,:], cat_embedding).to(device)
...
class MLPDiffusion(nn.Module):
    ...
    def forward(self, x, timesteps, cond, cat_embedding):
        emb = self.time_embed(timestep_embedding(timesteps, self.dim_t))
        for i in range(len(cat_embedding)):
            emb += cat_embedding[i](cond[i].long())
        x = self.proj(x.float()) + emb
        return self.mlp(x)

    def sample(self, x, timesteps, cond, cat_embedding):
        emb = self.time_embed(timestep_embedding(timesteps, self.dim_t)).repeat(x.shape[0], 1)
        for i in range(len(cat_embedding)):
            emb += cat_embedding[i](cond[:,i].long())
        x = self.proj(x.float()) + emb
        return self.mlp(x)
...
def Euler_Maruyama_sampling(model, T, N, P, device, cond, cat_embedding, snr = 0.1, M = 10):
    ...
    with torch.no_grad():
        for epoch in tqdm_epoch:
            ...
            grad = model.sample(X, time_step, cond, cat_embedding)
            ...
            X = X - ( f - (g**2) * ( model.sample(X, time_step, cond, cat_embedding) )  ) * step_size.to(device) + torch.sqrt(step_size).to(device)*g*torch.randn_like(X).to(device)
